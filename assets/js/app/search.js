...
var pageLoadCount = 0;
var hidden_search = $("#search").val();

function get_search_product(search, sortby, pageno, callback) {
    pageNo = pageno;
    $('.loading-container').removeClass('d-none');
    $.ajax({
        method: "post",
        url: site_url + "get-search-product",
        data: {
            search: search,
            sortby: sortby,
            pageno: pageno,
        ...
        success: function (response) {
            ...
            setTimeout(() => {
                ...
                $("#search_product").append(product_html);
                callback();
            }, 500);
        },
    });
...
$(document).on('change', '#sort_data_id', function () {
    ...
    isFetching = true;
    get_search_product(hidden_search, sort_id, 0, () => {
        isFetching = false;
    });
});
...
window.addEventListener('scroll', () => {
    ...
    if (scrollTop + clientHeight >= scrollHeight - 500) {
        if (pageNo < total_pages - 1) {
            ...
            sort_id = $("#sort_data_id").val();
            isFetching = true;
            get_search_product(hidden_search, sort_id, pageNo, () => {
                isFetching = false;
            });
        }
    }
...
window.addEventListener('load', () => {
    ...
    isFetching = true;
    get_search_product(hidden_search, '', 0, () => {
        isFetching = false;
    });
