...
if ($code == "1") {
	...
	try {
		...
		// echo "ease ".$search;
		$Exist = false;
		$stmt11 = $conn->prepare("SELECT odr.sno, odr.order_id, odr.status, odr.total_price, odr.create_date, up.full_name, sum(op.prod_price*op.qty), sum(op.cgst), sum(op.sgst), sum(op.shipping), sum(op.total)  FROM orders odr, user_profile up, order_product op WHERE odr.user_id = up.user_id AND odr.order_id = op.order_id AND odr.customer_name LIKE ? GROUP BY op.order_id ORDER BY odr.sno DESC");
