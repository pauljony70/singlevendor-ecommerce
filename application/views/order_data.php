          ...
          <div class="row">
            <div class="col-12 col-md-9">
              ...
              <div class="d-flex justify-content-between fs-6">
                <div class="fs-6 fw-medium"><?= count($order['order_products']) ?> Product(s)</div>
                <div class="fs-6 fw-medium"><?= price_format($order['order_details']['total_price']) ?></div>
              </div>
              <hr class="mt-2 mb-3">
              <?php
              $total_item = 0;
              ...
              <?php foreach ($order['order_products'] as $order_product) : ?>
                <?php
                $total_item += $order_product['qty'];
                ?>
                <div class="row mb-4">
                  <div class="col-3">
                    <a href="<?= base_url('productdetail/' . $order_product['prod_name'] . '/' . $order_product['prod_id']) ?>">
                      <img src="<?= MEDIA_URL . json_decode($order_product['prod_img'])[0] ?>" alt="<?= $order_product['prod_name'] ?>" class="w-100">
                    </a>
                  </div>
                  <div class="col-9">
                    <a href="<?= base_url('productdetail/' . $order_product['prod_name'] . '/' . $order_product['prod_id']) ?>" class="fs-6 fw-semibold text-dark"><?= $order_product['prod_name'] ?></a>
                    <div class="mt-1 fs-6">
                      <?php foreach (json_decode($order_product['prod_attr'], true) as $config_attr) : ?>
                    ...
                    <div class="fs-6 mt-4 mb-1">Qty: <?= $order_product['qty'] ?></div>
                    <div class="d-flex fs-6">
                      <div class="fw-medium"><?= price_format($order_product['prod_price']) ?></div>
                      <div class="ms-2 text-decoration-line-through fw-light"><?= price_format($order_product['prod_price']) ?></div>
                    </div>
                  </div>
                </div>
              <?php endforeach; ?>
              <div class="bg-secondary bg-opacity-25 p-3 mb-4">
                ...
                <div class="d-flex justify-content-between fs-6 fw-light mb-3">
                  <div>Subtotal (<?= $total_item ?> Items)</div>
                  <div><?= price_format($order['order_details']['total_price']) ?></div>
                </div>
