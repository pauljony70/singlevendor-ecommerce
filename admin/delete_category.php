...
if (!isset($_SESSION['admin'])) {
	...
	function inactiveProduct($id, $conns)
	{
		...
		$status = "inactive";
		$stmt11 = $conns->prepare("UPDATE product SET status =? WHERE prod_cat_id IN ( $id ) AND status != ?");
		$stmt11->bind_param("ss", $status, $status);
		$stmt11->execute();
		// echo " insert done ".$id;
		$rows = $stmt11->affected_rows;
		if ($rows > 0) {
			echo " Category Product's Become Inactive";
		} else {
	...
	if (isset($deletearray) && !empty($deletearray)) {
		...
		$stmt2 = $conn->prepare("DELETE FROM category WHERE cat_id IN( $deletearray ) OR parent_id IN( $deletearray )");
